...
async fn main() -> eyre::Result<()> {
    ...
    abigen!(
        StylusProvider,
        r#"[
            function run_compute(bytes calldata input) external view returns (bytes memory)
        ]"#
    );
    ...
    let provider = StylusProvider::new(address, client);
    let ciphertext = provider.run_compute(Bytes::from(vec![])).call().await;
